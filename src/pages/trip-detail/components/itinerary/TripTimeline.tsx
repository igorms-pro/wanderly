...
function TimelineActivityCard({
  ...
  const formatActivityCost = (a: Activity, defaultCurrency: string): string | null => {
    const min = a.cost_min_cents ?? a.cost_cents;
    const max = a.cost_max_cents ?? a.cost_cents;
    if (min == null && max == null) return null;
    const lo = min ?? 0;
    const hi = max ?? 0;
    const curr = a.currency || defaultCurrency;
  ...
  const getGoogleMapsUrl = (a: Activity): string | null => {
    if (a.lat != null && a.lon != null) {
      return `https://www.google.com/maps?q=${a.lat},${a.lon}`;
    }
    const name = (a.place_name || a.title || '').trim();
    if (name) {
            ...
            {isExpanded && (
              <>
                <span className="flex items-center gap-1">
                  ...
                  {formatActivityCost(activity, currency) || t('tripDetail.costNotSet')}
                </span>
