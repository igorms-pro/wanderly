...
const AUTH_STORAGE_KEY = 'voyagely-auth';
  ...
  persist(
    (set, get) => ({
      ...
      initializeAuth: async () => {
        ...
        // Never block the UI: if Supabase hangs, mark initialized after 5s so user sees login or rehydrated state
        const safetyTimeout = setTimeout(done, 5000);

        try {
          // CI/tests may run without env vars – skip then so tests don't hit real Supabase
      ...
      updateTrip: async (tripId, updates) => {
        try {
          ...
          if (updates.constraints !== undefined)
            updateData.constraints = updates.constraints ?? null;
      ...
      loadActivities: async (tripId) => {
        try {
          ...
          const mappedActivities: Activity[] = ((activities || []) as any[]).map(
            (activity: any) => ({
              id: activity.id,
